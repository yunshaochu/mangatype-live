...
export const translations = {
  zh: {
    ...
    splitDetectionByLines: "按 lines 自动拆分合并框",
    splitDetectionByLinesHint: "当检测 API 把相邻气泡合并成一个 block 时，尝试根据 text_blocks[].lines 二次拆成多个红框（实验性）。",
    preciseFill: "精确轮廓填充",
    preciseFillHint: "填充红框时，仅涂抹文字轮廓像素，而非整块矩形。需先扫描获取轮廓数据。",
    showContourPreview: "显示轮廓预览",
    showContourPreviewHint: "以橙色叠加显示识别到的文字轮廓，供参考。",
    showDetectionLines: "显示 lines 范围",
    showDetectionLinesHint: "叠加显示检测 API 返回的 text_blocks[].lines 行级区域，用来观察这些行框是否有参考价值。",
    useCharRects: "单字矩形模式",
    ...
    exportSkippedAsOriginal: "跳过图片导出原图",
    exportSkippedAsOriginalHint: "开启后，被标记为跳过的图片在下载、ZIP 和合并图层时导出原图；关闭则导出当前编辑结果。",
  ...
  en: {
    ...
    splitDetectionByLines: "Split Merged Blocks by Lines",
    splitDetectionByLinesHint: "When the detection API merges nearby bubbles into one block, try splitting it into multiple red boxes using text_blocks[].lines (Experimental).",
    ...
    showDetectionLines: "Show Detection Lines",
    showDetectionLinesHint: "Overlay the line-level polygons returned by text_blocks[].lines so you can evaluate whether they are useful.",
    ...
    exportSkippedAsOriginal: "Export Skipped As Original",
    exportSkippedAsOriginalHint: "When enabled, skipped images export as original in download, ZIP, and merge flows. When disabled, export current edited result.",
