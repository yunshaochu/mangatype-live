...
assert.equal(getAdjacentBubbleLayoutIndex({ activeLayoutIndex: 0, layoutVariants: [{ breakAfter: [2] }] } as any, 'next'), 1, 'next variant should move to the first extra candidate');
assert.deepEqual(
  buildActiveBubbleFontSizeUpdate(bubble, 1.4),
  { layoutVariants: [{ breakAfter: [2, 4], fontSize: 1.4 }] },
